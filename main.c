    ...
    uint8_t rx_byte = 0;
      ...
      while (1)
      {
          ...
          if (pid_trigger == 1)
          {
              ...
              // NOWA OBSŁUGA WYSYŁKI Z CRC:
                  char msg_buffer[128];
                  static int frame_counter = 0; // Licznik ramek
                  frame_counter++;
                  // 1. Składamy tekstową część wiadomości
                  int msg_len = sprintf(msg_buffer, "Ref:%.1f, Meas:%.1f, PWM:%.1f%%",
                                        target_lux, current_lux, current_pwm);

                  uint8_t crc_val = Compute_CRC8(msg_buffer, msg_len);

                      // 3. WYMUSZENIE BŁĘDU (co 10 ramka)
           //           if (frame_counter >= 10)
            //          {
              //            crc_val = 0xAA; // Wpisujemy błędną wartość (np. AA zamiast prawdziwej)
                //          frame_counter = 0; // Resetujemy licznik
                  //    }
